...
public class GestureAnalyser {
	...
	private int calcGesture() {
		...
		if (numFingers == 3) {
			...
			/*
			 * if (finalFingDist(0,1) > 2*(initialFingDist(0,1))) { return UNPINCH; } if (finalFingDist(0,1) <
			 * 0.5*(initialFingDist(0,1))) { return PINCH; }
			 */
		}
		if (numFingers == 4) {
			if (((-delY[0]) > (2 * Math.abs(delX[0]))) && ((-delY[1]) > (2 * Math.abs(delX[1])))
					&& ((-delY[2]) > (2 * Math.abs(delX[2]))) && ((-delY[3]) > (2 * Math.abs(delX[3])))) {
				return SWIPE_4_UP;
			}
			if (((delY[0]) > (2 * Math.abs(delX[0]))) && ((delY[1]) > (2 * Math.abs(delX[1])))
					&& ((delY[2]) > (2 * Math.abs(delX[2]))) && ((delY[3]) > (2 * Math.abs(delX[3])))) {
			...
			if (((-delX[0]) > (2 * Math.abs(delY[0]))) && ((-delX[1]) > (2 * Math.abs(delY[1])))
					&& ((-delX[2]) > (2 * Math.abs(delY[2]))) && ((-delX[3]) > (2 * Math.abs(delY[3])))) {
				return SWIPE_4_LEFT;
			}
			if (((delX[0]) > (2 * Math.abs(delY[0]))) && ((delX[1]) > (2 * Math.abs(delY[1])))
					&& ((delX[2]) > (2 * Math.abs(delY[2]))) && ((delX[3]) > (2 * Math.abs(delY[3])))) {
