...
public abstract class AbstractWheelTextAdapter extends AbstractWheelAdapter {
	...
	public static final int STATE_POSITIVE_VALUE = 1;

	public static final int STATE_NEGATIVE_VALUE = 2;
	...
	protected AbstractWheelTextAdapter(Context context, int itemResource, int itemTextResource) {
		...
		textColor[0] = DEFAULT_TEXT_COLOR;
	...
	/**
	 * @param statePositiveValue
	 * @param green
	 */
	public void setTextColor(int state, int green) {
		textColor[state] = green;
	}
	...
	protected int getColorForState(int state) {
		if (textColor[state] != 0)
			return textColor[state];
		else
			return textColor[STATE_DEFAULT_VALUE];
	}
	...
	public View getEmptyItem(int index, View convertView, ViewGroup parent) {
		...
		if (emptyItemResourceId == TEXT_VIEW_ITEM_RESOURCE && convertView instanceof TextView) {
			configureTextView(index, (TextView) convertView);
		}
	...
	protected void configureTextView(int index, TextView view) {
		int state = getStateForValue(index);
		view.setTextColor(getColorForState(state));
		view.setGravity(Gravity.CENTER);
		view.setTextSize(TypedValue.COMPLEX_UNIT_SP, textSize);
		view.setLines(1);
		view.setTypeface(Typeface.SANS_SERIF, Typeface.BOLD);

		int px = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 5, view.getResources()
				.getDisplayMetrics());
